...
class CIDR
{
     ...
     * @return String CIDR block.
     */
    public static function IPisWithinCIDR(string $ipinput, array $cidr_array)
    {
        foreach ($cidr_array as $cidr) {
            if (self::IPisWithinCIDR_single($ipinput, $cidr)) {
                return true;
            }
        }
        return false;
    ...
    public static function IPisWithinCIDR_single(string $ipinput, string $cidr): bool
    {
        $acidr = explode('/', $cidr);
        if (!isset($acidr[1])) {
            $acidr[1] = '32';
        }
        $cidr = self::alignedCIDR($acidr[0], self::CIDRtoMask((int)$acidr[1]));
        $cidr = explode('/', $cidr);
    ...
    public static function getIP($keys = ['HTTP_X_DDOSPROXY', 'HTTP_X_FORWARDED_FOR', 'HTTP_CLIENT_IP', 'REMOTE_ADDR'])
    {
        foreach ($keys as $key) {
    ...
    public function ip($keys =  ['HTTP_X_DDOSPROXY', 'HTTP_X_FORWARDED_FOR', 'HTTP_CLIENT_IP', 'REMOTE_ADDR']): mixed
    {
        return CIDR::getIP($keys);
    }

    public function matchesCIDR($ip, $cidr): mixed
    {
        return self::IPisWithinCIDR($ip, $cidr);
    }

    public static function run(&$request, &$result)
    {
        $result['cidr'] = new CIDR();
    }
}
